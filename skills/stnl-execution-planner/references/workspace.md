...
- `PLANNED_DRAFT` / `PLANNED_READY`: the plan set exists without tasks;
- `PENDING_REPLAN_DRAFT` / `PENDING_REPLAN_READY`: an append-only extension or pristine replacement is staged without changing task history;
...
- `AUXILIARY_BLOCKED`: a valid auxiliary `BLOCKED` resumes only the originating operation/slice;
- `RUNNER_INITIALIZATION_BLOCKED` / `RUNNER_RESULT_BLOCKED`: an active `Delegation Blocker` resumes only its named operation/slice;
