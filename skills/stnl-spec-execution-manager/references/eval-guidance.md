...
Use the named cases in `evals/eval-plan.md` before adopting a substantial change. Check requirements preservation, `SPEC_PATH` and slice normalization, derived artifacts, optional-context conflict blocking, minimal launcher contract, slice boundaries, upfront task materialization, relative paths, selective reading, evidence quality, independent validation, safe parallelization, closure boundaries, headers on internal artifacts, and absence of mandatory provider or model.
