...
- `canonical_id_gate`: headings are canonical and unique; the heading is the only ID authority; prefixes match files; metadata fields, order, arrays, statuses, impacts, and narrative sections satisfy the category schema; items contain no YAML, duplicate `id:`, explicit template placeholders, or optional `null`.
...
- `question_gate`: no question is `open` for readiness; feature and questions headers reflect open state; `open_questions` exactly matches the sorted open IDs; only open questions may carry `blocks`, and only active ACs may carry reciprocal `blocked_by`.
...
- `relationship_gate`: open-question `blocks` and active-AC `blocked_by` are bidirectionally exact; `blocked_by` never points to a non-open question; `linked_decision` resolves to a decision.
- `scope_gate`: included and excluded behavior prevent accidental expansion.
- `acceptance_criteria_gate`: a `ready` SPEC has at least one active, unblocked criterion. Active criteria are semantically reviewed as sufficiently concrete, observable, verifiable, and consistent with scope and rules; the deterministic parser checks structure, not keyword-based observability.
...
Any open question forces the feature header to `blocked`, even when `blocks: []`. A `ready` SPEC has no open question, broken reference, documentary gap, or active `blocked_by`. An active mitigated risk remains `active` and does not fail readiness merely by existing.
