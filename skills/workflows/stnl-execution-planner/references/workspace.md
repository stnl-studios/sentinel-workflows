...
Slice identity is semantic: use the smallest cohesive, observable, and validatable outcome or operational milestone that must be preserved as one execution unit. Frontend, backend, API, database, persistence, tests, telemetry, security, and integration work normally stay inside that Slice as Tasks. A separate Slice requires an independently observable capability, lifecycle/contract/state/validation or rollback boundary, real migration/cutover/rollout/compatibility state, or an explicitly required cross-slice or operational milestone; task volume, file count, technical affinity, and agent/context capacity do not create a boundary.
