...
In global `Expected areas` and detailed `Likely Areas`, every Markdown code span is reserved exclusively for one concrete filesystem path and is interpreted deterministically as a path claim. Compute each persisted path relative to the specific artifact containing it. Keep command names, subcommands, CLI verbs, flags, functions, methods, classes, symbols, variables, config or JSON keys, HTTP methods, endpoints, routes, protocols, schemas, packages, subsystems, conceptual labels, feature or behavior names, test descriptions, and non-claimed generic filenames in plain text outside code spans. Candidate validation rejects non-canonical, escaping, symlinked, execution-root, or accidental lifecycle-SPEC-local implementation targets; never reinterpret a rejected code span as prose, substitute project-root resolution, or rewrite a rejected claim heuristically.
