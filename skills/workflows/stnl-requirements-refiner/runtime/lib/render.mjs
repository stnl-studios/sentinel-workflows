...
function decisionFollowUpContext(question, interaction) {
  ...
  const contextSection = conflict === undefined
    ? `<section><h5>What this established</h5>${list(questionEstablishedContext(question), "Nothing has been established yet.")}</section>`
    : `<section class="conflict-context"><h5>Previous established context</h5>${list(conflict.previous_context, "No prior context recorded.")}<h5>New conflicting information</h5>${list(conflict.conflicting_context, "No conflicting context recorded.")}<p><strong>Current status:</strong> Conflict requires reconciliation</p></section>`;
...
const PROMPT_CONTRACT = Object.freeze({
  ...
  previousEstablishedPrefix: "Previous established context:\n",
  ...
  establishedPrefix: "What this established:\n",
...
function promptEntry(question, decision, maps) {
  ...
  if (followUp) {
    ...
    if (conflict === undefined) {
      lines.push(PROMPT_CONTRACT.establishedPrefix + (questionEstablishedContext(question).join("\n") || "none"));
    } else {
      lines.push(
        PROMPT_CONTRACT.previousEstablishedPrefix + (conflict.previous_context.join("\n") || "none"),
