...
8. Write the complete strict candidate JSON to a single-link ephemeral file outside `PROJECT_ROOT`, without `migration_provenance`, per-Question `migration_history`, or imported-answer exceptions. Execute `node "<SKILL_ROOT>/runtime/generate-refinement.mjs" INIT <PROJECT_ROOT> <CANDIDATE_MODEL> [REFINEMENT_PATH]`.
...
7. Recompute relationships, questions, constraints, assessment, carried context, and the conservative handoff across the full authority. Preserve canonical answers separately from the last human response; reopening a question appends a reopen event that preserves the challenged answer, reason, and current gaps. Historical attempt context is immutable, while current established context is a deterministic sequential projection: compatible `FOLLOW_UP_REQUIRED` rounds accumulate, `NO_MATERIAL_PROGRESS` preserves, `CONFLICTING_INFORMATION` removes only explicitly referenced current facts and keeps the conflict active, and a later `ACCEPTED` attempt replaces the projection with the final canonical interpretation. New Questions created after migration are native-v2 and cannot carry `migration_history` or use imported-answer exceptions. Open `ATTENTION`/`INFO` and bypassed findings may travel downstream.
