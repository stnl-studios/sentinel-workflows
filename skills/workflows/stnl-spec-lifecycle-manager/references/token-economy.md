...
`maintenance/runtime-context-budget.json` is the measured dependency and budget manifest. Its baseline is the pre-change `HEAD`; words use whitespace separation and bytes use UTF-8 file bytes. No tokenizer is required. `node scripts/test-runtime-context-budget.mjs` verifies dependencies, maintenance exclusion, ceilings, and required reductions while printing current results.
