...
import { filesystemComponentKey, ValidationError } from "./lifecycle/core.mjs";
import { parseStrictJson } from "./lifecycle/strict-json.mjs";

export { ValidationError };

export const DEFAULT_ROADMAP_PATH = "docs/roadmap";
export const MODEL_FILENAME = "roadmap.json";
...
export function isCanonicalPhysicalAlias(requested, physical) {
  ...
  return ["/etc", "/tmp", "/var"].some((prefix) => (
    (requested === prefix || requested.startsWith(`${prefix}/`))
    && physical === `/private${requested}`
  ));
}
...
export function normalizeRelativePath(value, label) {
  ...
  if (segments.some((segment) => {
    const lower = filesystemComponentKey(segment);
  ...
  if (segments.some((segment) => REPOSITORY_METADATA_NAMES.has(filesystemComponentKey(segment)))) {
    throw new ValidationError(`${label} identifies a repository metadata path`);
  }
  return segments.join("/");
}

export function normalizeRoadmapPath(value = DEFAULT_ROADMAP_PATH) {
  return normalizeRelativePath(value, "ROADMAP_PATH");
}
...
export async function resolveProjectRoot(value) {
  ...
  if (!isCanonicalPhysicalAlias(requested, physical)) throw new ValidationError(`PROJECT_ROOT must use its canonical physical path: ${requested}`);
  return physical;
}
...
export async function resolveInsideProject(projectRoot, relativeValue, label) {
  ...
  const candidate = path.resolve(projectRoot, ...relative.split("/"));
  const boundary = path.relative(projectRoot, candidate);
  if (boundary === ".." || boundary.startsWith(`..${path.sep}`) || path.isAbsolute(boundary)) {
    throw new ValidationError(`${label} escapes PROJECT_ROOT`);
  }
  await assertNoSymlinkComponents(candidate, label);
  const metadata = await lstatOrNull(candidate);
  if (metadata !== null) {
    const physical = await fs.realpath(candidate);
    if (!isCanonicalPhysicalAlias(candidate, physical)) throw new ValidationError(`${label} is not a canonical physical path: ${relative}`);
  }
  return { relative, absolute: candidate, metadata };
}

export async function resolveRoadmap(projectRootValue, roadmapPathValue = DEFAULT_ROADMAP_PATH) {
  const projectRoot = await resolveProjectRoot(projectRootValue);
  const roadmapPath = normalizeRoadmapPath(roadmapPathValue);
  const resolved = await resolveInsideProject(projectRoot, roadmapPath, "ROADMAP_PATH");
  return {
    projectRoot,
    roadmapPath,
    roadmapRoot: resolved.absolute,
    roadmapMetadata: resolved.metadata,
...
export async function requireSingleLinkRealFile(filePath, label, maximumBytes = MAX_MODEL_BYTES) {
  ...
  if (!isCanonicalPhysicalAlias(path.resolve(filePath), await fs.realpath(filePath))) {
    throw new ValidationError(`${label} contains a non-canonical physical component: ${filePath}`);
  }
  return metadata;
}

export async function readStrictJsonFile(filePath, label = "roadmap model") {
...
export function sanitizeDiagnostic(message, projectRoot) {
  return String(message).replaceAll(projectRoot, ".");
}
