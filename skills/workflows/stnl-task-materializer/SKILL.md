...
After reading each approved Slice and before rendering its Tasks, perform a late semantic boundary gate. Decompose engineering work into Tasks while preserving one outcome/milestone per Slice. If the decomposition exposes independent observable outcome clusters with their own lifecycle, contract, state, validation, risk/rollback, or hidden cross-cluster dependency, or makes ownership impossible to keep coherent, stop with `NEEDS_REPLAN` and return a concrete executable diagnostic for `REPLAN_REASON`; publish no candidate or live bytes. Route a discovered SPEC boundary problem through the existing requirements/lifecycle handoff instead of hiding it in additional task files.

This gate is not a task-count, file-count, layer-count, complexity, or agent/context heuristic. A semantically correct vertical Slice may have many Tasks. Frontend, backend, API, persistence, database, tests, telemetry, security, and ordinary integration remain internal Tasks unless the approved Slice itself names an independent operational milestone. Do not create generic Foundation, Integration, Stabilization, Tests, Telemetry, Cleanup, Follow-up, or remaining-work Tasks/Slices as a deposit for work that belongs to the outcome.
