...
Check that no plan obligation was lost and no task was invented. Verify fidelity, coverage, granularity, order, dependencies, objective results, tests, slice isolation, absence of work belonging elsewhere, consistency between global and detailed tasks, and economy of context for execution. Treat the approved Slice as one outcome/milestone: frontend, backend, API, persistence, database, tests, telemetry, security, and ordinary integration are normally internal Tasks. Many Tasks, files, or technical areas do not by themselves make a boundary problem, and agent/context limits never justify a semantic split. If the materialized checklist reveals independent observable outcomes, lifecycle/contract/state/validation/rollback boundaries, hidden cross-cluster dependencies, or impossible ownership that the approved plan cannot reconcile, return `NEEDS_REPLAN` with an executable boundary diagnostic rather than creating a generic Foundation, Integration, Stabilization, Tests, Telemetry, Cleanup, Follow-up, or remaining-work Slice. Correct task artifacts directly when the approved plan already determines the answer.
