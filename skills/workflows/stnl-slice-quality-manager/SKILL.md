...
Every delegated payload includes `VALIDATION_HARNESS_PATH=<SKILL_ROOT>/runtime/run-validation-session.mjs`, formal `round:null`, exact current task-relative subjects and baseline, the preceding attempt evidence ID, and a replay origin ID only when a code-regression reproduction is proposed. The harness verifies the legal formal invocation and exact prior attempt before copying or executing anything. Source symlinks are admitted by final canonical containment in the canonical project root, then preserved or rebased into the isolated copy; broken, ambiguous, or canonically external targets fail closed before commands. An external launcher is admitted only from an inherited canonical PATH bin, with a package boundary and interpreter derived inside the same concrete installation; the harness authenticates and executes a sparse read-only snapshot and binds its before/after identity into evidence and replay. Every verification command runs through that harness in an isolated operating-system sandbox; neither context executes it directly against live state.

Persist exact inline `Evidence provenance` without reconstruction. Invalid, side-effecting, stale, unclean or non-equivalent replay evidence can only create `BLOCKED` and cannot create a finding, PASS, ACCEPTED or Effective Validation Base. An authenticated harness pre-check `INVALID/INFRASTRUCTURE_BLOCKED/NONE` result is persisted in a `Kind: infrastructure` Delegation Blocker with no `attempt-NN`; evidence from a validation that entered check execution is persisted in its attempt. New findings include immutable `Kind: implementation_defect|code_regression` and `Evidence identity` bound to their origin attempt. `code_regression` requires an equivalent replay anchored to a prior `VERIFIED` persisted evidence record; an invalid reproduction has conclusion `NONE`. On PASS/ACCEPTED, provenance subjects exactly own the Effective Validation Base files and are rechecked before candidate publication.
