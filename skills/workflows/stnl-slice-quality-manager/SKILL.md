...
Start validation in a new independent delegated session with no inherited conversation history, using the platform launcher supplied by the caller. Resolve the launcher's concrete `SPEC_PATH` once before delegation, require that exact absolute canonical path to exist and pass the official preflight, and copy that byte-identical value into the runner payload. Never replace it with the execution root, selected task, candidate root, CWD-relative path, placeholder, or reconstructed path. Send only operation, that exact `SPEC_PATH`, derived execution root, selected slice, plan/task paths, current requirements fingerprint and plan revision, compact implementation/findings evidence, active and historical finding summary, existing Validation Attempt summary, changed scope and diff, overlaps, the bounded terminal context above only when this is the last open slice, and strictly necessary additional context. Never send conversation history or full logs.
