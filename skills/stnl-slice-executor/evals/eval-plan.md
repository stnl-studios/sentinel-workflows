...
4. After valid preconditions and implementation or correction, each manual operation invokes the configured independent runner at least once and at most three times, never a fourth time or through an unbounded loop, while the main context runs no verification command or fallback.
...
9. `TESTS_FAIL` in rounds one and two permits only evidence-backed in-scope correction before recheck; a third failure stops without a fourth invocation/correction, enters the operation-specific `*_RETRY_EXHAUSTED` state, and permits only explicit `VALIDATE_SLICE` next.
...
12. No auxiliary status starts `VALIDATE_SLICE` automatically or changes formal validation authority; third failure requires that explicit next operation.
...
14. Every operation preflights deterministic layout, state, fingerprint/revision, active records, and third-failure phase before mutation or delegation.
