...
## Runner role integration and bounded full campaign (2026-09-26)

Entry was published `feature/atlas-p0` HEAD `f6feb4b4b1af072d7dd9bf15caec5ba468067363` (parent `79d2723ac90f0bb65c44cb861e65b735e9a18e51`), clean functional tree, no active run or reservation, `git diff --check` PASS, and ledger 294/350. The earlier A runner had made zero tool calls before claiming it could not read the executor skill. No attempted file access or sandbox denial supported that claim. A second, real seatbelt check later demonstrated that the independent runner can read its snapshot and workspace while Main, source checkout, sibling run, and private-home canaries remain denied; no credential content was used as evidence.

The confirmed integration defect was role configuration: `developerInstructions` from the trusted TOML was previously concatenated to the operation prompt. The runner also saw the executor skill in its catalog; whether that alone caused its claim is unproven. The pinned SDK and CLI are both 0.154.0. The SDK transport now passes the versioned runner contract through public per-instance `config.developer_instructions` and disables only runner-side copied workflow skills through public `skills.config`; author instances keep their skills. The operation request keeps the managed mechanical envelope and exact semantic payload separate, and no longer exposes the producer serializer or duplicates the runner contract. The mirrored runner contract v9 reconciled authority, paths, candidate and producer ownership; the runtime still derives identities, fingerprints, records, freshness and publication. [Official Codex configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference) documents these public configuration keys. The runner was not granted Main or private-home access.

One bounded GPT-5.6-Luna/medium maintenance worker patched the mirrored contracts and their checker/tests; the principal integrated the SDK, adapter, contract and packaging tests. Before the full, runner/adapter and contract tests passed 14/14, execution contracts 123/123, packaging 23/23, `check-contracts` PASS, `validate.sh --no-smoke` PASS, benchmark `verify` PASS and `git diff --check` PASS. A real focal in `benchmark-temp/focal-runner-20260926-integration/` used a copied historical A fixture with official `RUNNER_RESULT_BLOCKED` same-operation recovery, current frozen snapshot, managed bridge, broker, SDK runner, ChatGPT/openai restricted home and the historical semantic payload. It returned `TESTS_PASS` after actual 6/6 focused and 12/12 complete tests; deterministic producer insertion and strict candidate validation reached `IMPLEMENTED_AWAITING_VALIDATION` without changing the live task. The permission probe used the actual seatbelt boundary. Worker and focal cost one turn each, reaching 296/350.

The official `run --full` was `benchmark-temp/run-20260926175538-41f0e9a2/`, 17:55:39–19:04:32 UTC. It froze base SHA `f6feb4b4b1af072d7dd9bf15caec5ba468067363`, functional diff `sha256:f55f1e65a08f587713503f6a2d19bcab8d4e309dc9b310b10d8691cec9bc132d`, source identity `sha256:be764d05b86a37c88247d8719a3164c7f3abfa49de10ab20928bbf733fde28e4`, snapshot `sha256:7759dd83e28c1ecb8d169bc711ace385ee4f8c6bde0db77dc1a937475bfd8f9d`, profile `production-v2`. A passed all 10 operations (10 main/4 runner), implemented and validated two slices, reached `COMPLETE`, closed the SPEC and finalized PASS. B passed all 12 operations (12 main/7 runner), implemented and validated three slices, reached `COMPLETE`, closed and finalized PASS. Its first slice-01 runner check found a real legacy priority failure, then the automatic correction and second check passed. C passed INIT, PLAN, reviews, task materialization and first implementation, but its first formal `VALIDATE_SLICE` blocked (7 main/2 runner). All cases had final `node --test` exit 0, expected model profiles, source/snapshot integrity PASS and terminal private-home removal. A/B finalizer exit 0; C finalizer exit 1. Full result is `A:PASS B:PASS C:BLOCKED`, never a global PASS, at ledger 338/350.

The C runner correctly identified missing storage validation coverage and returned `NEEDS_FIX`, but it used noncanonical `F-001` and prose in `findingDispositions`. The producer rejected `finding references must be none or a canonical finding set`; the quality manager preserved the rejected response/candidate and published a truthful malformed-output delegation blocker, official state `RUNNER_RESULT_BLOCKED`. This is a semantic response grammar defect, not a permission, SDK transport or failing test command. The v9 contract had not stated the producer's `finding-01` / `finding-01=active` grammar. After the full ended, the mirrored contract was advanced to v10 with exact finding IDs, ordering, dispositions and a new-NEEDS_FIX example; its checker and regression were updated. No old run artifact was rewritten.

The post-patch focal `benchmark-temp/focal-findings-20260926/` copied C's blocked workspace, used its official same-operation `VALIDATE_SLICE slice-01` preflight and original payload, and froze a new source identity `sha256:026813c3bb86bfd162a7581bf98cd197bd3072a3e0ed4ab18594f378b589fa32` / snapshot `sha256:840ee72e50639b4ba8e94905c09c9e90ac9f2f01f9e190da73f89761b0b380f8`. The live runner returned `NEEDS_FIX` with `finding-01` and `finding-01=active`, reproduced duplicate-ID acceptance, and preserved the workspace. The actual producer accepted the original response; a separate candidate with the structured finding and resolved delegation blocker prepared `attempt-01`, passed strict validation as `VALIDATION_NEEDS_FIX`, and left live task bytes unchanged. This focal did not publish a formal C outcome and is not combinable with A/B from the earlier functional revision. It cost one runner turn, yielding 339/350 and 11 free with no reservations.

After the v10 patch, runner/adapter/contract tests passed 15/15, execution contracts 123/123, packaging 23/23, `validate.sh --no-smoke`, benchmark `verify`, and `git diff --check` passed. Full-run raw telemetry totals 38,032,280 input and 263,354 output tokens across all 29 main and 13 runner turns, with cached/reasoning values included subcategories; the two focals report 97,869/2,106 and 72,921/2,034 input/output respectively. Worker and root maintainer usage are unavailable. These SDK counters are not subscription charges. The mission used 45 controlled turns from 294 to 339: one worker, two focals and 42 full turns.

Checkpoint: `PAUSED_LOCAL_BUDGET`. The manager supports resume only for a stopped single-case focal, not this terminal full, whose C private home was removed. Repeating A/B/C on the v10 revision is required for same-revision global PASS. The prior full used 42 turns before C's correction path; a normal C findings cycle would require additional main and runner turns. Eleven available turns are insufficient for a responsible new full. Baseline remains `NOT_YET_ESTABLISHED`, G6 `NOT_YET_PROVEN`, G2/G3 `PARTIAL`, and P0 open. The minimum user decision is an explicit extension of the existing 350-turn ceiling for one new full with findings recovery; do not reset the ledger or combine old A/B with a new C focal. No commit or push was made.

## P0 operational closure attempt on published v10 (2026-09-26)

Entry preflight confirmed `feature/atlas-p0` at published HEAD `f1dd41c0c0318b60e31a9227d07c43f3bf3b3f68`, parent `f6feb4b4b1af072d7dd9bf15caec5ba468067363`, clean functional tree, no active run or worker, zero reservations, and `git diff --check` PASS. The existing ledger was exactly 339/350; the single authorized +55 extension was recorded append-only as 350→405, leaving 66. No reset, renumbering, commit, or push occurred. The first functional action was the fresh `run --full`.

The full is `benchmark-temp/run-20260926195045-13f38459/`, 19:50:45–20:11:54 UTC (21m09s). It froze the clean v10 source identity `sha256:026813c3bb86bfd162a7581bf98cd197bd3072a3e0ed4ab18594f378b589fa32`, empty functional diff `sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, and snapshot `sha256:840ee72e50639b4ba8e94905c09c9e90ac9f2f01f9e190da73f89761b0b380f8`. A passed SPEC_INIT, PLAN, REVIEW_PLAN, MATERIALIZE_TASKS, and REVIEW_TASKS, then blocked in EXECUTE_SLICE slice-01 at official `AUXILIARY_BLOCKED`; B and C were NOT_RUN under the A gate. A used 6 main and 1 runner turn, moving the ledger 339→346/405. The independent runner did real discovery and `npm test` (9/9), but its valid semantic response was `BLOCKED`: the author had invoked the adapter directly with `{}`, omitting `automaticCheckRound`. The producer preserved the result and published the truthful auxiliary blocker; there was no transport, auth, quota, candidate, or test failure. The case's final `node --test` passed, while its finalizer exited 1 for BLOCKED. Production-v2 profile mismatches were empty; ChatGPT/openai restricted isolation, source/snapshot integrity and private-home removal passed. Full normalized usage: main 6,085,843 input / 56,268 output; runner 70,982 input / 1,486 output. Root usage is unavailable.

The causal patch is confined to `agents/codex/runtime/validation-runner.mjs` and its runner/broker tests. The adapter now rejects absent or ambiguous automatic rounds before dispatch for EXECUTE_SLICE/APPLY_FINDINGS, and its CLI refuses direct managed invocation so the configured pathless bridge remains the entry point. The local regression reproduces `{}` rejection and accepts one canonical round; managed VALIDATE_SLICE remains unaffected. Focused runner/context/contract tests passed 23/23, `bash scripts/validate.sh --no-smoke` PASS, benchmark `verify` PASS, and `git diff --check` PASS. The live boundary focal `benchmark-temp/focal-round-20260926/` copied the blocked A workspace without publishing it, froze patched source identity `sha256:1c0b29b8c61c4c629814848dc57541a32db30b507b07afd239f1571aba794742` / functional diff `sha256:049d0fce3b2ed920a665f4125a69e322020233f445f175ead3cd83082458d60b` / snapshot `sha256:b98f0dc64cb756df358bc0eabea5fb19441040faca854984e2064231433b1258`, and used the managed bridge with `automaticCheckRound=1/3`. The independent runner returned `TESTS_PASS` after focused `node --test test/todo-service.test.mjs` and full `npm test`, both exit 0, with ChatGPT/openai restricted isolation. It consumed one runner turn (347); focal usage was 46,177 input / 1,870 output. No GPT-6-Luna maintenance worker was used; principal usage is unavailable.

Checkpoint: `PAUSED_LOCAL_BUDGET`, not provider quota. The ledger is 347/405, 58 free, zero reservations, no active run or worker. The current A plan has three slices; the prior B and C plans also had three. A normal fresh full needs about 18 A + 18–19 B + 22 C = 58–59 controlled turns using observed operation counts and C's known findings recovery, before any extra retry or new finding. There is no responsible margin under the absolute 405 ceiling, so a second full was not started merely to test the patch. No same-revision A/B/C convergence exists; baseline remains `NOT_YET_ESTABLISHED`, G6 `NOT_YET_PROVEN`, G2/G3 `PARTIAL`, and P0 open. Preserve both full and focal raws. The minimum next decision is a separately authorized budget above this mission's 405 cap, followed by one fresh full on a published fixed source revision; the current source patch remains unstaged and uncommitted for user review.
