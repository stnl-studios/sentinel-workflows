...
A solicitação deve informar uma dessas operações, o envelope mecânico do adapter e um payload semântico contendo evidências relevantes, escopo alterado e contexto adicional opcional. Em managed mode, não repita `SPEC_PATH`, workspace, operação, slice, snapshot, adapter, bridge ou preflight no payload semântico: o envelope é a autoridade e qualquer divergência retorna `BLOCKED` antes de dispatch. `EXECUTE_SLICE` e `APPLY_FINDINGS` também informam a rodada automática atual como `1/3`, `2/3` ou `3/3`; o contador pertence à operação manual atual e não autoriza uma quarta invocação. Normalize o número para `slice-NN`. Entrada ausente, ambígua, operação diferente, operação em batch ou solicitação de paralelização retorna `BLOCKED`.

## Envelope mecânico do adapter

Quando a sessão for iniciada pelo managed runner bridge, o envelope mecânico do adapter é a autoridade para `SPEC_PATH`, workspace, operação, slice, snapshot, preflight, adapter e bridge. Não reconstrua identidade a partir de `CODEX_HOME`, caminho da skill instalada, nome da thread, CWD ou sufixos de private home. Se envelope e contexto discordarem, retorne `BLOCKED` antes de qualquer leitura de artefato ou verification command. O bridge encaminha o objeto semântico byte-a-byte; não o reserializa, traduz ou complementa.
...
Em `EXECUTE_SLICE`, `APPLY_FINDINGS` e `VALIDATE_SLICE`, capture o `HEAD` atual com `git rev-parse HEAD` antes de preencher o campo semântico `head`. Se o comando tiver exit 0 e uma linha stdout com exatamente 40 hex minúsculos, use essa linha mesmo quando o Git da Apple emitir avisos de cache/FSEvents em stderr; esses avisos não substituem o resultado do comando. Se não houver SHA verificável, retorne `BLOCKED` com causa concreta; nunca retorne `TESTS_PASS` com `head` vazio, placeholder ou diagnóstico em vez de SHA. Em `VALIDATE_SLICE`, reconcilie o manifesto com mudanças originais, correções, efeitos adicionais necessários, remoções, testes relevantes e arquivos finais necessários ao `PASS`. Em `Tested state` de qualquer operação e no manifesto final da slice, todo caminho é normalizado e relativo ao diretório do artefato detalhado final `tasks/slice-NN.md` selecionado. Liste esses caminhos relativos únicos em ordem lexicográfica, com SHA-256 minúsculo do conteúdo ou `REMOVED` quando ausente. Não retorne `PASS` com manifesto vazio, incompleto, duplicado, malformado ou inconsistente e não invente hashes. A representação fileless canônica não é um manifesto vazio: use exact `none`, `Fileless reason` objetivo, commands autoritativos e evidência observável; não invente path ou hash.
