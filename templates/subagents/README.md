...
Todo verification command passa pelo `runtime/run-validation-session.mjs` distribuído com `stnl-slice-executor` e `stnl-slice-quality-manager`. Antes de copiar ou executar, o runtime confirma operação, slice, round e prior evidence ID contra o lifecycle atual. Ele copia o projeto para temp do sistema, aplica sandbox do SO sem rede, mantém source e `execution/` protegidos, permite somente HOME/TMP e `writePaths` declarados, captura provenance/fingerprints e limpa a sessão. Ambiente sem sandbox suportada bloqueia sem fallback. Evidência inválida, stale, side-effecting ou replay não equivalente não pode sustentar finding ou sucesso. Manifestos preservam cada identidade de arquivo; agregação de diretório é apenas derivada. Replay de regressão precisa resolver uma evidência histórica `VERIFIED` persistida e provar equivalência material antes de executar.
...
O singleton auxiliar registra `Pending automatic round: N/3`. Uma nova chamada da mesma operação, quando implementação ou correção e escopo já foram persistidos e o único bloqueio final é de inicialização, retoma diretamente na delegação, na rodada ainda não consumida. Não reimplementa, não reaplica findings, não duplica checklist, diff, evidência ou blocker, não reinicia identificadores e não cria rodada antes de a sessão iniciar. O fallback no contexto principal é proibido.
