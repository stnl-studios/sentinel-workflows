...
Delegue obrigatoriamente to `@agent-stnl-validation-runner` in a fresh session sem histórico da conversa. Send only operation/slice, `SPEC_PATH`, authority/revision, changed scope/diff, compact evidence and project references. Never send skill/install/runtime/harness paths, filenames, schemas, secrets, logs or envelope.
