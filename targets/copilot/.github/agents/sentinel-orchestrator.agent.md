...
Route only: planner -> developer approval -> test-planner -> developer approval -> coder -> validator -> reviewer -> finalizer. Confirm current phase, current slice, artifact availability, and approval state. Do not inspect source code or judge another agent's work.
