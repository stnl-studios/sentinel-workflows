...
Validate the current slice against the current slice package, approved plan, approved test plan, acceptance criteria, and relevant DoD. Inspect only scoped changes and evidence. Re-run or verify approved commands where needed.
...
Do not edit code/tests/contracts, spec workspace files, fix failures, review broad architecture, or close the spec. Reject vague claims and a bare “tests passed.” Missing mandatory evidence, path violations, or unverifiable results cannot pass.
