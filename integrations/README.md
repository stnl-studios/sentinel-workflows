...
Este agente copiável, barato e isolado faz discovery/planning independente e, quando exigido, assessment pós-harness para as skills `stnl-slice-executor` e `stnl-slice-quality-manager`. Ele não localiza nem invoca o harness.
...
- `EXECUTE_SLICE`: o contexto principal implementa; o runner devolve um plano; a skill dona chama seu bridge e o harness produz a evidence/status auxiliar.
- `APPLY_FINDINGS`: o contexto principal corrige findings autorizados; o runner planeja checks; a skill dona chama seu bridge e o harness produz a evidence/status auxiliar.
- `VALIDATE_SLICE`: o runner planeja, a skill dona chama bridge/harness e o runner faz um assessment independente posterior vinculado ao evidence ID; a skill dona valida e publica o candidate.

O runner recebe somente contexto lógico do projeto, sem skill root, instalação ou filename de runtime. O bridge é invocado pela skill dona a partir de sua localização carregada, devolve resumo compacto e mantém envelope/logs em receipt temporário opaco; o owner usa o helper empacotado para materializar candidate sem reserializar evidence. Não existe fallback para execução direta pelo contexto principal ou pelo runner.

Depois da implementação ou correção inicial, cada invocação manual válida de `EXECUTE_SLICE` ou `APPLY_FINDINGS` chama o runner no mínimo uma vez e no máximo três vezes: a primeira chamada é obrigatória e pode ser seguida por até duas rechecagens. Não existe bypass por mudança simples ou por provável ausência de checks; o runner sempre faz a descoberta independente, inclusive quando planeja zero comandos com justificativa objetiva de não aplicabilidade. O bridge/harness, e não o planner, deriva então `TESTS_NOT_APPLICABLE` da execução selada sem comandos. Em `TESTS_FAIL` nas duas primeiras rodadas, o contexto principal persiste a evidência, corrige somente a falha objetiva dentro do escopo aprovado, registra a correção e o escopo atualizado e chama o runner novamente. `TESTS_PASS`/`TESTS_ACCEPTED`/`TESTS_NOT_APPLICABLE` entram em `IMPLEMENTED_AWAITING_VALIDATION` ou `FINDINGS_CORRECTED`; `BLOCKED` após entrada em check entra em `AUXILIARY_BLOCKED`, enquanto pre-check autenticado de infraestrutura fica em `RUNNER_RESULT_BLOCKED` sem criar check nem consumir rodada; só a mesma operação/slice retoma. A terceira falha entra em `IMPLEMENTATION_RETRY_EXHAUSTED` ou `FINDINGS_RETRY_EXHAUSTED`: `VALIDATE_SLICE` é a única próxima operação da slice, sem reentrada do executor até o veredito formal. Não existe quarta chamada, loop ilimitado ou transição automática para `VALIDATE_SLICE`.
...
O contrato canônico atual é `stnl-validation-runner/v11`, uma mudança incompatível no papel do subagente, com handshake exato contra o harness preservado `stnl-validation-harness/v10`. Toda request física é construída pelo bridge depois da validação do plano e carrega ambos os protocolos; cada comando declara `executionEnvironment` explicitamente. Ausência, versão desconhecida, mixed version ou capability stale bloqueia antes de verification. A provenance v10 inclui protocolo e receipt do harness nos fingerprints e evidence ID; runner e owner não os reconstruem. Registros históricos com runner v10 continuam legíveis, mas não autorizam execução, append ou replay novos.

O runner v11 descobre e planeja checks lógicos; somente o packaged harness executa fisicamente build, test, lint, typecheck, compiler, validator ou regression. O adapter Claude remove Bash porque as leituras fornecidas bastam ao planejamento. O adapter Codex permanece `read-only`, mas a plataforma ainda pode oferecer shell de leitura; isso não concede autoridade de execução e o fluxo correto não depende dele. O runner não recebe localização do harness. A skill dona chama o bridge relativo ao `SKILL.md` carregado, e só evidence selada pelo harness pode avançar candidate/lifecycle.

O contrato preserva `Gate assessments` nos registros existentes. Falha externa independente não bloqueia; causalidade desconhecida exige assessment e não REPLAN. Revalide o working tree, mesmo sem commit, antes de decidir se o blocker anterior ainda existe. Um bypass explícito de gate opcional concreto usa `BYPASS_GATE=<registro-NN>/gate-NN` e `BYPASS_REASON=<risco aceito>`; é vinculado à evidence/autoridade e resulta em TESTS_ACCEPTED/ACCEPTED. Publique e distribua juntos os owners, bridge, harness, capability e adapters correspondentes.
