...
Esses caminhos são de fonte, não de instalação. O layout nativo de destino permanece `.codex/agents/stnl_validation_runner.toml` e `.codex/agents/stnl_spec_context_scout.toml` no Codex, e `.claude/agents/stnl-validation-runner.md` e `.claude/agents/stnl-spec-context-scout.md` no Claude Code. A futura distribuição deve materializar somente os dois adapters da plataforma escolhida nesses destinos; o mecanismo de instalação não faz parte deste repositório nesta fase. Nunca misture os adaptadores das duas plataformas no mesmo projeto e não altere configurações globais do usuário.
...
Copie somente os três launchers da plataforma usada a partir de `templates/prompts/` para o mecanismo de prompts do projeto, preservando os nomes acima. Os adapters canônicos vêm de `integrations/codex/agents/` ou `integrations/claude-code/agents/`, conforme a plataforma. `execution-plan.md`, `execution-replan.md`, `execution-plan-review.md`, `execution-tasks.md`, `execution-tasks-review.md` e `execution-close.md` continuam compartilhados.
...
Os dois adaptadores implementam o mesmo contrato. O runner pode ler o escopo necessário e solicitar verification commands somente ao harness, mas não edita código, testes, requisitos, planos ou tasks; não implementa, não corrige, não persiste, não finaliza, não cria subagentes e não faz commit. Artefatos transitórios de build ou teste são permitidos somente nos `writePaths` isolados declarados; efeito fora desses limites invalida a evidência.
