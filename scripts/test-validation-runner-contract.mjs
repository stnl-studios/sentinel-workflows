...
const cases = [
  ...
  ["missing check status", "R006_VERDICTS", (root) => replaceBoth(root, "STATUS_CHECKS=TESTS_PASS|TESTS_ACCEPTED|TESTS_FAIL|TESTS_NOT_APPLICABLE|BLOCKED", "STATUS_CHECKS=TESTS_PASS|TESTS_FAIL|BLOCKED")],
  ["missing formal status", "R006_VERDICTS", (root) => replaceBoth(root, "STATUS_VALIDACAO=PASS|ACCEPTED|NEEDS_FIX|BLOCKED", "STATUS_VALIDACAO=PASS|BLOCKED")],
  ...
  ["pre-check blocker loses authenticated classification", "R018_PROVENANCE", (root) => replaceBoth(root, "provenance.classification=INFRASTRUCTURE_BLOCKED", "provenance.classification=NONE")],
  ["replay equivalence removed", "R019_REPLAY", (root) => replaceBoth(root, "`CODE_REGRESSION` exige `replayOriginEvidenceId`", "CODE_REGRESSION aceita qualquer replay")],
  ["file identity collapsed", "R020_EVIDENCE_IDENTITY", (root) => replaceBoth(root, "diretório, basename ou agregado nunca substitui identidades de arquivos", "diretório pode substituir arquivos")],
  ...
  ["command failure becomes N/A", "R016_NOT_APPLICABLE", (root) => replaceBoth(root, "Falha relevante de verification command é `TESTS_FAIL`", "Falha de verification command é `TESTS_NOT_APPLICABLE`")],
  ...
  ["PASS loses zero exit", "R006_VERDICTS", (root) => replaceBoth(root, "`TESTS_PASS` exige que os comandos obrigatórios da slice tenham exit code zero", "`TESTS_PASS` aceita qualquer exit code")],
  ...
  ["manifest allows empty", "R008_MANIFEST", (root) => replaceBoth(root, "Não retorne `PASS` ou `ACCEPTED` com manifesto vazio", "Retorne `PASS` com manifesto vazio")],
