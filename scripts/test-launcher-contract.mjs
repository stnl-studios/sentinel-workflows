...
const cases = [
  ...
  ["Codex inherits history", "slice-execute-codex.md", (text) => text.replace("sem histórico", "com histórico"), "L016_TRANSPORT"],
  ["forwards conversation", "slice-apply-findings-codex.md", (text) => text.replace(/(?:não envie|nunca envie) histórico da conversa/iu, "envie histórico da conversa"), "L012_CHECK_DELEGATION"],
  ...
  ["INIT deterministic owner serializer removed", "spec-init.md", (text) => text.replace(/Depois de concluir o candidate INIT isolado[\s\S]*?rejeição\./u, "Contexto adicional (opcional):"), "L051_INIT_HEADER_SERIALIZATION"],
  ...
  ["materializer deterministic task path serializer removed", "execution-tasks.md", (text) => text.replace("Os code spans `Likely Areas` dos detailed plans aprovados são as seleções semânticas.", "Os caminhos das tasks são texto livre."), "L043_TASK_PATH_SERIALIZATION"],
  ["runner deterministic evidence serializer removed", "slice-execute-codex.md", (text) => text.replace("__RUNNER_EVIDENCE_SERIALIZER__", "SERIALIZER_PATH"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ["runner complete evidence record serializer removed", "slice-execute-codex.md", (text) => text.replaceAll("registro canônico completo", "registro").replaceAll("registro canônico", "registro"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ["runner complete response serializer removed", "slice-execute-codex.md", (text) => text.replaceAll("registro canônico", "registro"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ["execute incomplete final producer invocation", "slice-execute-codex.md", (text) => text.replaceAll("node \"$RUNNER_EVIDENCE_SERIALIZER\" --execution-bundle --operation <requested-operation> --workspace \"{{MANAGED_WORKSPACE}}\" --spec-path \"{{SPEC_PATH}}\" --slice \"{{SLICE}}\" --semantic-response-file", "--execution-bundle --operation <requested-operation> --semantic-response-file"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ["runner response capture removed", "slice-validate-codex.md", (text) => text.replaceAll("captureRunnerResponse", "manual extraction"), "L049_DETERMINISTIC_RESPONSE_CAPTURE"],
  ["runner response schema removed", "slice-execute-codex.md", (text) => text.replaceAll("schema fechado", "schema ausente"), "L050_STRUCTURED_RESPONSE_SCHEMA"],
  ["runner Codex schema-binding contract removed", "slice-execute-codex.md", (text) => text.replace("anexa ao argv real o schema fechado", "aplica fora do argv o schema fechado"), "L050_STRUCTURED_RESPONSE_SCHEMA"],
  ["runner validation candidate preparer removed", "slice-validate-codex.md", (text) => text.replace("--prepare --spec-path", "--separate-output-modes"), "L046_VALIDATION_BUNDLE"],
  ["validation candidate copy returned to model", "slice-validate-codex.md", (text) => text.replace("driver oficial já preparou `CANDIDATE_EXECUTION_ROOT` como cópia completa e byte-identical", "o producer copiará a execution tree depois"), "L047_VALIDATION_CANDIDATE_COPY"],
  ["materializer incomplete candidate tree guard removed", "execution-tasks.md", (text) => text.replace("cópia completa e isolada", "cópia parcial"), "L045_COMPLETE_TASK_CANDIDATE"],
  ["materializer deterministic candidate root preparation removed", "execution-tasks.md", (text) => text.replaceAll("__MATERIALIZER_TASK_CANDIDATE_PREPARER__", "WRONG_PREPARER"), "L047_CANDIDATE_ROOT_PREPARATION"],
  ...
  ["planner deterministic path serializer removed", "execution-plan.md", (text) => text.replace("The global plan `Expected areas` code spans are the model's semantic physical-target selections.", "The plan targets are prose."), "L042_PLAN_PATH_SERIALIZATION"],
  ["plan reviewer deterministic path serializer removed", "execution-plan-review.md", (text) => text.replace("The candidate global plan `Expected areas` code spans are the reviewed semantic physical-target selections.", "The reviewed targets are prose."), "L042_PLAN_PATH_SERIALIZATION"],
  ["planner deterministic candidate header preparer removed", "execution-plan.md", (text) => text.replaceAll("__PLANNER_PLAN_CANDIDATE_PREPARER__", "MISSING_PREPARER"), "L043_PLAN_CANDIDATE_HEADER"],
  ["plan reviewer deterministic candidate header preparer removed", "execution-plan-review.md", (text) => text.replaceAll("__PLANNER_PLAN_CANDIDATE_PREPARER__", "MISSING_PREPARER"), "L043_PLAN_CANDIDATE_HEADER"],
  ...
  ["Codex numeric history", "slice-validate-codex.md", (text) => text.replace("sem histórico", "com histórico"), "L016_TRANSPORT"],
  ...
  ["missing Codex runner launcher", "slice-apply-findings-codex.md", (text) => text.replaceAll("__RUNNER_INVOCATION_HELPER__", "LAUNCHER_PATH"), "L007_PLATFORM_IDENTITY"],
  ...
  ["execute producer canonical task derivation removed", "slice-execute-codex.md", (text) => text.replaceAll("taskArtifact", "wrongArtifact"), "L028_TESTED_STATE_PATHS"],
  ["execute producer physical identity removed", "slice-execute-codex.md", (text) => text.replace("verifica `realpath`, calcula `path.relative` com `/`", "calcula `path.relative` com `/`"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ["execute producer canonical record serialization removed", "slice-execute-codex.md", (text) => text.replace("calcula o próximo identificador e emite diretamente o registro canônico", "calcula o próximo identificador e emite diretamente o registro"), "L044_DETERMINISTIC_EVIDENCE_SERIALIZATION"],
  ...
  ["execute canonical output gate removed", "slice-execute-codex.md", (text) => text.replace("Exija forma estrita do payload semântico", "Permita forma livre do payload semântico"), "L038_EXECUTE_OUTPUT_GATE"],
  ["execute tuple grammar guard removed", "slice-execute-codex.md", (text) => text.replace("`commands` como array de `{command, exit}`", "`commands` como texto livre"), "L039_EXECUTE_TUPLE_GRAMMAR"],
  ["execute literal field sequence guard removed", "slice-execute-codex.md", (text) => text.replace("exatamente as chaves lowerCamelCase da operação", "chaves em ordem livre"), "L040_EXECUTE_FIELD_SEQUENCE"],
  ["execute scalar field shape guard removed", "slice-execute-codex.md", (text) => text.replace("todos os valores escalares como strings", "valores com qualquer shape"), "L041_EXECUTE_FIELD_SHAPE"],
  ...
  ["validate producer-owned preflight removed", "slice-validate-codex.md", (text) => text.replace("O producer determinístico launcher-owned insere como primeiro command", "O producer determinístico launcher-owned pode omitir o primeiro command"), "L020_EXACT_COMMANDS"],
