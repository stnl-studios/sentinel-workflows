...
const cases = [
  ...
  ["refinement path removed", "requirements-refinement-init.md", (text) => text.replace(/^REFINEMENT_PATH=.*\n/mu, ""), "L004_INPUTS"],
  ["refinement authority removed", "requirements-refinement-reconcile.md", (text) => text.replaceAll("refinement.json", "other.json"), "L022_REFINEMENT_BOUNDARY"],
  ["refinement browser state promoted", "requirements-refinement-init.md", (text) => text.replace("não é autoridade Sentinel", "é autoridade Sentinel"), "L022_REFINEMENT_BOUNDARY"],
  ["refinement creates downstream authority", "requirements-refinement-reconcile.md", (text) => text.replace("Não crie, altere nem invoque uma SPEC, Roadmap ou workflow downstream", "Crie uma SPEC, Roadmap e workflow downstream"), "L022_REFINEMENT_BOUNDARY"],
  ["refinement loses manual handoff", "requirements-refinement-init.md", (text) => text.replace("handoff manual", "handoff automático"), "L022_REFINEMENT_BOUNDARY"],
